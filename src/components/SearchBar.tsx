      ...
      <CommandDialog open={searchModal.isOpen} onOpenChange={toggleModal}>
        ...
        <CommandList>
          ...
          {isPending ? (
            ...
            results?.map(product => (
              <CommandGroup className="capitalize" key={product.id} heading={product.subCategoryName} >
                <CommandItem onSelect={() => { toggleModal(); router.push(`/product/${product.id}`) }} >{product.name}</CommandItem>
              </CommandGroup>
            )
            ))}
        </CommandList>
